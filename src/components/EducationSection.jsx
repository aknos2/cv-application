...
export default function EducationSection({ onSave, initialData }) {
  ...
  const confirmDelete = (e) => {
    ...
    if (educationEntries.length > 1 && entryToDelete) {
      setEducationEntries(
        educationEntries.filter((entry) => entry.id !== entryToDelete),
      );
    }
          ...
          {Array.isArray(savedData) &&
            savedData.map((entry) => (
              <div key={entry.id} className="saved-info education-section">
                <div>
                  <p>
                    <span className="highlight-word">School Name</span>:{" "}
                    {entry.schoolName}
                  </p>
                  <p>
                    <span className="highlight-word">Field</span>:{" "}
                    {entry.studyField}
                  </p>
                </div>
                <div className="from-to-wrap">
                  <p>
                    <span className="highlight-word">From</span>:{" "}
                    {entry.dateFrom}
                  </p>
                  <p>
                    <span className="highlight-word">To</span>: {entry.dateTo}
                  </p>
                </div>
                <hr />
              </div>
