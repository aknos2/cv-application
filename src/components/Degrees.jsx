...
export default function DegreesSection({ onSave, initialData }) {
  ...
  const { savedData, isEditing, showForm, handleSaveButton } =
    useFormSave(null);
  ...
  return (
    <div className={`container ${isSaved ? "saved" : ""}`}>
      <h2 className="degree-title-wrapper">
        Degrees <spam id="degree-title-spam">(if applicable)</spam>
      </h2>
      ...
      <Button
        type="button"
        className="add-btn"
        aria-label="add new degree"
        onClick={() => addRow(setDegreeRows, degreeRows)}
        text="+"
      />
